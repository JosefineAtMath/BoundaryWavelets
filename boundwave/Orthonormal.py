...
def Integral(J,k,l,WaveletCoef,phi):
    '''
    This function calculates the intergal (16) numericly.
    INPUT:
        J:
            int: The scale.
        k:
        ...
        phi:
            numpy.float64: The phi function, can be made with pywt.Wavelet(wavelet).wavefun(level=15)
    OUTPUT:
        int: Integ:
            ** To be done **
    '''
...
def M_AlphaBeta(alpha,beta,J,WaveletCoef,InteMatrix,Side):
    ...
    INPUT:
        ...
        WaveletCoef:
            ...
            For Daubechies 2 they can be found using np.flipud(pywt.Wavelet('db2').dec_lo).
        InteMatrix:
            numpy.float64: A matrix with the values for the integrals calculated
            with the function integral() for k and l in the interval [-2*a+2,0]
            or [2**J-2*a+1,2**J-1]
        Side:
            str: 'L' for left interval boundary and 'R' for right interval boundary
    OUTPUT:
        M:
    ...
    if Side=='L':
        ...
        for k in Interval:
            j = 0
            for l in Interval:
                M += BW.InnerProductPhiX(alpha,0,k,Moment)*BW.InnerProductPhiX(beta,0,l,Moment)*InteMatrix[i,j]
                j += 1
            i += 1
    elif Side=='R':
        ...
        for k in Interval:
            j = 0
            for l in Interval:
                M += BW.InnerProductPhiX(alpha,0,k,Moment)*BW.InnerProductPhiX(beta,0,l,Moment)*InteMatrix[i,j]*2**(-J*(alpha+beta))
                j += 1
            i += 1
    else:
...
def OrthoMatrix(J,WaveletCoef,phi):
    ...
    for i in range(-2*a+2,1):
        ...
        for j in range(-2*a+2,i+1):
            InteL[k,l] = Integral(J,i,j,WaveletCoef,phi)
            InteL[l,k] = InteL[k,l]
            l         += 1
        k += 1
    ...
    for i in range(2**J-2*a+1,2**J):
        ...
        for j in range(2**J-2*a+1,i+1):
            InteR[k,l] = Integral(J,i,j,WaveletCoef,phi)
            InteR[l,k] = InteR[k,l]
            l         += 1
        k += 1
    for i in range(a):
    ...
    AR    = 2**(J/2)*np.dot(np.dot(np.transpose(V),np.diag(1/S)),np.transpose(U))
    return AL,AR
