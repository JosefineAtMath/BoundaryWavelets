...
def ReconBoundary(WaveletCoef,J,Wavelet,phi,N=15):
    '''
    This function reconstructs a 1D signal in time from its wavelet coefficients,
    using boundary wavelets at the edge.\n
    INPUT:
        WaveletCoef:
...
def ReconMirror(WaveletCoef,J,Wavelet,phi,N=15):
    '''
    This function reconstructs a 1D signal in time from its wavelet coefficients,
    using mirroring of the signal at the edge.\n
    INPUT:
        WaveletCoef:
            numpy.float64: The wavelet coefficients, must sum to sqrt(2). 
            For Daubechies 2 they can be found using np.flipud(pywt.Wavelet('db2').dec_lo).
