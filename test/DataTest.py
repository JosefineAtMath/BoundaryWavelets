...
def TestPlot(Name='Data',Row=1,Section=214,J=7,N=12,Wavelet='db3'):
    ...
    calculated and printed and all three signals are plotted in the same figure.
    INPUT:
        Name:
        ...
        Section:
            int: Which section of the data to use. The samples that will be used
            are: [Section*2**N:Section*2**N+2**N]
        J:
            int: The scale.
        N:
...
def Test(Name='Data',Row=1,J=7,N=12,Wavelet):
    ...
    OUTPUT:
        Result:
            float64: 2D array. the firt row is the difference between the original
            signal and the reconstruction using Boundary wavelet. The second row
            is the difference between the original signal and the reconstruction
            using mirrored extension. The third row is the first row minus the 
            second row. there is one collumn for each section of the signal.
    '''
    data    = sp.loadmat(Name)
    phi     = pywt.Wavelet(Wavelet).wavefun(level=14)[0][1:]
    phi     = phi[::2**(14-N)]
