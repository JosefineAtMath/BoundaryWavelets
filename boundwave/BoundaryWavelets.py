...
def DownSample(x,Shift,N,J,zero=True):
    """
    This is a function which dilates a signal. Make sure there are enough samples
    for it to make sense.\n
    INPUT:
        x:
            numpy.float64: 1d numpy array, signal to be dilated.
        Shift:
            int: Time shift before dilation (for now it only supports integers).
        N:
            int: Number of samples per 'time' unit.
        J:
            int: The scale to make. Non-negative integer.
        zero=True:
            bool: If true, it concatenates zeros on the signal to retain the original
            length.
    OUTPUTS:
        y:
...
def Moments(WaveletCoef,n):
    ...
    OUTPUT:
        moments:
            numpy.float64: An 1d array with the moments.
    '''
    Moments = np.ones(n+1)
    for l in range(1,n+1):
        for k in range(len(WaveletCoef)):
            for m in range(l):
                Moments[l] += 1/((2**l-1)*np.sqrt(2))*WaveletCoef[k]*binom(l,m)*(k+1)**(l-m)*Moments[m]
    return Moments
...
def InnerProductPhiX(alpha,J,k,Moments):
    ...
    INPUT:
        ...
        Moments:
            numpy.float64: A 1d array of moments for phi, up to power alpha.
            Can be calculated using the function moments().
    OUTPUT:
        i:
...
def BoundaryWavelets(phi,J,WaveletCoef,AL=None,AR=None):
    ...
    OUTPUT:
        xj:
            numpy.float64: 2d numpy array with the boundary funtions in the columns.
    '''
    ...
    for b in range(a):
        for k in range(len(kLeft)):
            xj[:int((2*a-1+kLeft[k])*2**(-J)*OneStep),b] += InnerProductPhiX(b,J,kLeft[k],Moment)*PhiLeft[k,:int((2*a-1+kLeft[k])*2**(-J)*OneStep)]
            xj[int(2**(-J)*kRight[k]*OneStep):,a+b]      += InnerProductPhiX(b,J,kRight[k],Moment)*PhiRight[k,int(2**(-J)*kRight[k]*OneStep):] 
